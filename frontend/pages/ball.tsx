...
`
const DELETE_BALL = gql`
mutation DeleteBall($id:String!){
    deleteBall(id:$id){
        data
    }
  }`
export default function Ball() {
    ...
    const { loading, error, refetch } = useQuery(GET_BALLS, {
        onCompleted: data => {
            if (data) {
                setData(data)
            }
    ...
    const [deleteBallMutation, { data: BallDeleteData, loading: DeleteLoading, error: DeleteError }] = useMutation(DELETE_BALL,
        {
            onCompleted: data => {
    ...
    return (
        <div className="container mt-5">
            ...
            <Table striped bordered hover>
                ...
                <tbody>
                    {
                        stateData && stateData.balls.map((item: any, index: any) =>
                            <tr key={index}>
                                <td>{item.name}</td>
                                <td>{item.age}</td>
                                <td>{item.breed}</td>
            ...
            <CustomModal show={show} handleClose={handleClose} data={chooseData} name="ball" />
        </div>
    );
}
